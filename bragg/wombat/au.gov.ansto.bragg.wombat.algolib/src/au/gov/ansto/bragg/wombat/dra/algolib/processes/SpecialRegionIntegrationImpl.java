...
public class SpecialRegionIntegrationImpl extends HIPDProcessor  implements SpecialRegionIntegration{
	
	@Override
	protected Signal processNew(Signal in) {
		double[][] inData = in.dataAs(double[][].class);
		double[] extremals = MaskSectionBounds(inData, hid.beamX, hid.beamY,position);
		...
		double[][] y = findMaskArea(inData,  numSlices, extremals[0], extremals[1], hid.beamX, hid.beamY, position);
