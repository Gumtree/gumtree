 ...
 * @version 2.0
 */
public class VerticalIntegration implements ConcreteProcessor{
	...
	public Boolean process() throws Exception{
		...
		if (verticalIntegration_skip) verticalIntegration_output = verticalIntegration_input;
			...
			IArray variance_array = ((Plot) verticalIntegration_input).getVariance().getData();
...
//							verticalIntegration_region);
			...
			verticalIntegrate(
					sourceData, variance_array,verticalIntegration_minDist, verticalIntegration_maxDist,
					contrib_mask, resultArrays);
			...
			PlotFactory.addDataToPlot(verticalIntegration_output, resultName, resultArrays[0], "Integrated counts", "Counts", resultArrays[1]);
...
//		return result;
	...
	public void verticalIntegrate(IArray data, IArray variances,
			double minDist, double maxDist, IArray mask, IArray[] returned_data) throws InvalidRangeException {
		...
		for(int ttheta=0;ttheta<dataShape[dataRank-1];ttheta++) {
			...
			{
				double tth_value = tth_iter.getDoubleNext();
				String velem = String.valueOf(tth_value);
				...
				returned_data[0].setDouble(o_index, returned_data[0].getDouble(o_index)+tth_value);
			...
			// Finished single theta loop: now normalise to number of counts
			for(int i=0;i<oarray_shape[0];i++) {
				...
				if(nentry[i] == 0) returned_data[0].setDouble(o_index, 0);
					...
					returned_data[0].setDouble(o_index, returned_data[0].getDouble(o_index)/nentry[i]);
					returned_data[1].setDouble(ov_index, returned_data[1].getDouble(ov_index)/nentry[i]);
				}
			} 
		}
	}
