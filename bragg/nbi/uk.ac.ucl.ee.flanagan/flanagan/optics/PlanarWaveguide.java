...
public class PlanarWaveguide{
    ...
    public void calcCoreFilmRefractiveIndicesEstimatedGap(){
        ...
        // Vector to store sum of squares on each gap distance decrement
        ...
        while(test){
            ...
            if(coreRI!=coreRI){
                ...
                vec.addElement(new Double(coreRI));
                vec.addElement(new Double(sumOfSquares));
                numberOfDecrements++;
    ...
    public void plotFittedDispersionCurve(String legend){
            ...
            while(testModes){
                ...
                for(int i=0; i<this.numberOfTEmeasurements; i++){
                    if(this.measurementsTE[i][3]==pOrder){
                        ...
                        vecTE.addElement(new Double(this.measurementsTE[i][0]));
                        vecTE.addElement(new Double(this.measurementsTE[i][1]));
                    }
                }
                vecTE.insertElementAt(new Integer(pOrder), 2*pOrder);
                vecTE.insertElementAt(new Integer(pNumber), 2*pOrder+1);
        ...
        Vector<Object> vecTM = null;
        int pOrderNumberTM = 0;
        int pOrdersCheckedTM = 0;
        if(this.numberOfTMmeasurements>0){
            vecTM = new Vector<Object>();
            boolean testModes = true;
            int pOrder = 0;
            int numberTestedPositive = 0;
            while(testModes){
                ...
                for(int i=0; i<this.numberOfTMmeasurements; i++){
                    if(this.measurementsTM[i][3]==pOrder){
                        pNumber++;
                        numberTestedPositive++;
                        vecTM.addElement(new Double(this.measurementsTM[i][0]));
                        vecTM.addElement(new Double(this.measurementsTM[i][1]));
                    }
                }
                vecTM.insertElementAt(new Integer(pOrder), 2*pOrder);
                vecTM.insertElementAt(new Integer(pNumber), 2*pOrder+1);
        ...
        int plotNumber = 0;
        // TE mode curves
        int vecIndex = 2*(pOrdersCheckedTE+1);
        int vecHeaderIndex = 1;
        double tempD = 0.0D;
        int tempI = 0;

        if(this.numberOfTEmeasurements>0){
            int testVec = 0;
            int vecSize = vecTE.size();
            while(testVec<vecSize){
                // Check mode number has associated experimental data
                tempI = ((Integer)vecTE.elementAt(vecHeaderIndex)).intValue();
                testVec++;
                if(tempI>0){
                    modeType[atCurveNumber] = "TE";
                    modeType[atCurveNumber+1] = "TE";
                    modeNumber[atCurveNumber] = ((Integer)vecTE.elementAt(vecHeaderIndex-1)).intValue();
                    ...
                    for(int i=0; i<tempI; i++){
                        tempThick[i] = ((Double)vecTE.elementAt(vecIndex++)).doubleValue();
                        tempRefra[i] = ((Double)vecTE.elementAt(vecIndex++)).doubleValue();
                        testVec += 2;
                    }
                    double[] log10TempThick = (double[])tempThick.clone();
        ...
        if(this.numberOfTMmeasurements>0){
            int testVec = 0;
            int vecSize = vecTM.size();
            while(testVec<vecSize){
                // Check mode number has associated experimental data
                tempI = ((Integer)vecTM.elementAt(vecHeaderIndex)).intValue();
                testVec++;
                if(tempI>0){
                    modeType[atCurveNumber] = "TM";
                    modeType[atCurveNumber+1] = "TM";
                    modeNumber[atCurveNumber] = ((Integer)vecTM.elementAt(vecHeaderIndex-1)).intValue();
                    testVec++;
                    modeNumber[atCurveNumber+1] = modeNumber[atCurveNumber];
                    ...
                    double[] tempRefra = new double[tempI];
                    for(int i=0; i<tempI; i++){
                        tempThick[i] = ((Double)vecTM.elementAt(vecIndex++)).doubleValue();
                        tempRefra[i] = ((Double)vecTM.elementAt(vecIndex++)).doubleValue();
                        testVec += 2;
                    }
                    double[] log10TempThick = (double[])tempThick.clone();
                    for(int i=0; i<tempI; i++)log10TempThick[i] = Fmath.log10(tempThick[i]);
