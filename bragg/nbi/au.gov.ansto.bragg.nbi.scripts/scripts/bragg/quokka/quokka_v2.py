...
org.gumtree.control.model.PropertyConstants.ControllerState
...
#     controller = sics.getSicsController()
    ...
    while timeout > 0:
        ...
        count = 0
        while not control.get_status().equals(status) and count < gap:
            time.sleep(dt)
            count += dt
        
#         if control.get_status().equals(status):
#             break
#         else:
...
def isInterruptException(e):
    return isinstance(e, SicsInterruptException) or ('interrupted' in str(e).lower())
...
#    return False
    ...
    def getHistmemTextstatus(name):
        ...
        while True:
            ...
            except (Exception, SicsInterruptException) as e:
...
def doAcquisition(info):
    ...
    waitUntilSicsIs(control.ServerStatus.EAGER_TO_EXECUTE)
...
def determineDetRates(samples, timeout=60.0):
    ...
    startHistmem()
    waitUntilSicsIs(control.ServerStatus.COUNTING, timeout=60)
    try:
        time.sleep(1.0)
...
#     commandStatus = scanController.getCommandStatus()
    counter = 0.
    timeout = 60.
    while not scanController.getCommandStatus().equals(ControllerState.IDLE) :
#         if counter > timeout :
...
#             except:
#                 pass
            if not scanController.getCommandStatus().equals(ControllerState.BUSY) :
                raise Exception("Time out on syncExecute() where status did not changed whiling execution")
            else :
                statusChanged = True;
        if scanController.getCommandStatus().equals(ControllerState.BUSY) :
            statusChanged = True
    ...
    t_unchange = 120
    c_unchange = 0
    while scanController.getCommandStatus().equals(ControllerState.BUSY) :
        time.sleep(stime)
        count += stime
        c2 += stime
#         statusData = scanController.getStatusController().getValue().getStringData()
...
#             count = 0.
    ...
    if not scan_err is None:
        if 'interrupted' in scan_err.lower():
            raise SicsInterruptException('user interrupted')
