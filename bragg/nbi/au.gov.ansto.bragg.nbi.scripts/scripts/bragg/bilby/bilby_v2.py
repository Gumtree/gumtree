...
def dhv(val = None):
    if not val is None:
        if val.upper() == 'UP':
            dhv1_up = control.get_raw_value('dhv1 upper')
            dhv2_up = control.get_raw_value('dhv2 upper')
            res = control.run_command('drive dhv1 ' + str(dhv1_up) + ' dhv2 ' + str(dhv2_up))
            if res.find('Full Stop') >= 0:
                raise Exception, res
        elif val.upper() == 'DOWN':
            dhv1_down = control.get_raw_value('dhv1 lower')
            dhv2_down = control.get_raw_value('dhv2 lower')
            res = control.run_command('drive dhv1 ' + str(dhv1_down) + ' dhv2 ' + str(dhv2_down))
            if res.find('Full Stop') >= 0:
                raise Exception, res
    else:
        dhv1_up = control.get_raw_value('dhv1 upper')
        dhv1_down = control.get_raw_value('dhv1 lower')
        dhv1 = control.get_raw_value('dhv1')
        dhv2_up = control.get_raw_value('dhv2 upper')
        dhv2_down = control.get_raw_value('dhv2 lower')
        dhv2 = control.get_raw_value('dhv2')
