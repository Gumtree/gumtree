const TITLE_TEXT = "SICS Administration for ";
const PAR_INSTRUMENT_ID = "inst";

const URL_PREFIX = "sicsadmin/";
...
const STAGE_GROUP_TITLE = "SE Motor Groups";
const KEY_OTHER_GROUP = "OTHER_MOTOR_GROUPS";
const OTHER_GROUP_TITLE = "Other Motor Groups";
const KEY_OPTIONAL_MENU = "OPTIONAL_MENU_GROUPS";
const OPTIONAL_MENU_TITLE = "Optional Menu Groups";
const KEY_SELECTED_GROUP = "SELECTED_GROUP";
const KEY_ANCILLARY_GROUP = "ANCILLARY";
...
//		this.motorGroups;
	...
	load() {
		...
		$.get(obj.url, function(data) {
			...
			if (e.status == 401) {
				window.location = 'signin.html?redirect=sicsAdmin.html?inst=' + _inst;
			} else {
	...
	createMenuUi() {
		...
		if (KEY_STAGE_GROUP in obj.model) {
			...
			obj.model[KEY_STAGE_GROUP] = groupModel;
			obj.editorModel[KEY_STAGE_GROUP] = groupEditorModel;
		}
//		obj.motorGroupsModel = KEY_STAGE_GROUP in obj.editorModel ? obj.editorModel[KEY_STAGE_GROUP] : {};
		_motorGroups = new MotorGroups(KEY_STAGE_GROUP, groupModel, groupEditorModel, $sideBar, STAGE_GROUP_TITLE);
		_motorGroups.createMenuUi();
		
		var otherGroupModel = {};
		var otherGroupEditorModel = {};
		if (KEY_OTHER_GROUP in obj.model) {
			otherGroupModel = obj.model[KEY_OTHER_GROUP];
			otherGroupEditorModel = obj.editorModel[KEY_OTHER_GROUP];
		} else {
			obj.model[KEY_OTHER_GROUP] = otherGroupModel;
			obj.editorModel[KEY_OTHER_GROUP] = otherGroupEditorModel;
		}
		_otherGroups = new MotorGroups(KEY_OTHER_GROUP, otherGroupModel, otherGroupEditorModel, $sideBar, OTHER_GROUP_TITLE);
		_otherGroups.createMenuUi();

		var optionalMenuModel = {};
		var optionalMenuEditorModel = {};
		if (KEY_OPTIONAL_MENU in obj.model) {
			optionalMenuModel = obj.model[KEY_OPTIONAL_MENU];
			optionalMenuEditorModel = obj.editorModel[KEY_OPTIONAL_MENU];
		} else {
			obj.model[KEY_OPTIONAL_MENU] = optionalMenuModel;
			obj.editorModel[KEY_OPTIONAL_MENU] = optionalMenuEditorModel;
		}
		_optionalMenu = new MotorGroups(KEY_OPTIONAL_MENU, optionalMenuModel, optionalMenuEditorModel, $sideBar, OPTIONAL_MENU_TITLE);
		_optionalMenu.createMenuUi();

		var ancillaryModel = {};
		var ancillaryEditorModel = {};
		if (KEY_ANCILLARY_GROUP in obj.model) {
			ancillaryModel = obj.model[KEY_ANCILLARY_GROUP];
			ancillaryEditorModel = obj.editorModel[KEY_ANCILLARY_GROUP];
		} else {
			obj.model[KEY_ANCILLARY_GROUP] = ancillaryModel;
			obj.editorModel[KEY_ANCILLARY_GROUP] = ancillaryEditorModel;
		}
		_ancillaryGroup = new AncillaryGroup(KEY_ANCILLARY_GROUP, ancillaryModel, ancillaryEditorModel, $sideBar);
		_ancillaryGroup.createMenuUi();
	}
...
class MotorDBModel extends AbstractModel {
	...
	getSEMotorList() {
		return Object.keys(this.SEDevices);
	}
	
	getAllMotorList() {
		return Object.keys(this.allDevices);
	}
				...
				$.each(mc, function(aid, axis) {
					$.each(axis, function(mid, motor) {
						if (ID_PROP_SAMPLESTAGE in motor) {
							...
							if (Boolean(val)) {
								obj.SEDevices[mid] = motor;
							}
						} 
						obj.allDevices[mid] = motor;
					});
				});
			});
//			obj.createMenuUi();
...
//		const selectedMotor = _additionalModel.getSelectedMotor();
		...
		$.each(Object.keys(this.SEDevices), function(idx, mid) {
			const motor = obj.SEDevices[mid];
...
//					obj.clearDirtyFlag();
					...
					_otherGroups.applyChange();
					_ancillaryGroup.applyChange();
					_historyBar.reload();
					_saveButton.reset();
...
class MotorGroups extends AbstractDevice {
	constructor(did, model, editorModel, $parentUi, title) {
//	constructor(model, $ul) {
		super(did, model, editorModel, $parentUi);
//		this.model = model;
//		this.$ul = $ul;
		this.title = title;
		this.seOnly = title === STAGE_GROUP_TITLE;
		this.groupTitles = {};
		this.checkboxes = {};
		this.links = {};
		this.oldSelected = null;
...
//		this.groupEditor = {};
	...
	createMenuUi() {
		...
		var faIcon;
		var html = '<div class="class_a_motor_group" href="#"><h6 class="sidebar-subheading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 ">' 
				+ '<span class="class_span_mc_name" >' + obj.title + '</span>'
				+ '<span id="id_span_add" class="float_right clickable"><i class="fas fa-plus"></i> </span></h6></div>';
		obj.$menuHeader = $(html);
		
		obj.newGroupModal = new NewGroupModal(obj.$menuHeader.find('#id_span_add'), obj);
		obj.newGroupModal.init();
		...
		$.each(Object.keys(obj.editorModel), function(idx, gid) {
			...
			if (obj.seOnly) {
				...
				html = '<li class="nav-item class_li_subitem"><div class="menu_left">'
				+ '<a class="nav-link class_a_group" href="#" data-toggle="tooltip" data-placement="top" title="' + gid + '">' 
				+ title + '</a></div></li>';
			}
			const $li = $(html);
...
//			}
	...
	addNewGroup(gid, desc) {
		...
		var html;
		if (obj.seOnly) {
			html = '<li class="nav-item class_li_subitem"><div class="menu_left"><input type="checkbox" id="id_group_' + newGid + '" class="class_SE_checkbox">'
	...
	loadGroup(gid) {
		...
		const stageMotors = Object.keys(_motorDBModel.SEDevices);
...
//		this.rootEditor.hide();
		...
		if (!editor) {
			editor = new MotorGroupEditor(obj, gid, curConfig, obj.title === STAGE_GROUP_TITLE);
...
class AncillaryGroup extends AbstractDevice {
	...
	createMenuUi() {
		...
		var faIcon;
		var html = '<div class="class_a_motor_group" href="#"><h6 class="sidebar-subheading d-flex justify-content-between align-items-center px-3 mt-4 mb-1 ">' 
				+ '<span class="class_span_mc_name" >Ancillary Devices</span>'
				+ '<span id="id_span_add" class="float_right clickable"><i class="fas fa-plus"></i> </span></h6></div>';
		obj.$menuHeader = $(html);
		
		_newAncillaryModal = new NewAncillaryModal(obj.$menuHeader.find('#id_span_add'));
		_newAncillaryModal.init();
		
		obj.$menuUl = $('<ul class="nav flex-column class_ul_folder"></ul>');
//		const selectedGroup = _additionalModel.getSelectedGroup();
		$.each(obj.editorModel, function(aid, isEnabled) {
			const checkedText = isEnabled ? "checked" : "";
			html = '<li class="nav-item class_li_subitem"><div class="menu_left"><input type="checkbox" id="id_group_' + aid + '" class="class_SE_checkbox" ' + checkedText + '>'
				+ '<a class="nav-link class_a_group" href="#" data-toggle="tooltip" data-placement="top" title="' + aid + '">' 
				+ aid + '</a></div></li>';
			const $li = $(html);
			obj.addDeviceUi(aid, $li.find('input:checkbox'));
			obj.$menuUl.append($li);
			obj.configMenuDict[aid] = $li;
		});
		obj.$menuHeader.append(obj.$menuUl);
		obj.$parentUi.append(obj.$menuHeader);
	}
	
	addDeviceUi(aid, $checkbox) {
		if ($checkbox.is(':checked')) {
			this.oldSelected = aid;
		}
		this.checkboxes[aid] = $checkbox;
		const obj = this;
		$checkbox.change(function() {
			if ($checkbox.is(':checked')) {
				obj.setEnabled(aid, true);
			} else {
				obj.setEnabled(aid, false);
			}
		});
		
	}

	setEnabled(aid, isEnabled) {
		const obj = this;
		obj.editorModel[aid] = isEnabled;
	...
	checkDirty() {
		const obj = this;
		var isDirty = false;
		$.each(obj.editorModel, function(aid, value) {
			if (aid in obj.model) {
				const oldVal = obj.model[aid];
	...
	addNewDevice(aid) {
		...
		if (newAid in obj.editorModel) {
			StaticUtils.showError("Failed to add new ancillary device: id already exists, " + newAid);
			return;
		}
		obj.editorModel[newAid] = true;
		
		const html = '<li class="nav-item class_li_subitem"><div class="menu_left"><input type="checkbox" id="id_group_' + newAid + '" class="class_SE_checkbox" checked>'
			+ '<a class="nav-link class_a_group" href="#" data-toggle="tooltip" data-placement="top" title="' + newAid + '">' 
			+ newAid + '</a><span class="class_span_mc_control"><i class="fas fa-minus"></i> </span></div></li>';
		const $li = $(html);
		obj.addDeviceUi(newAid, $li.find('input:checkbox'));
		obj.$menuUl.append($li);
		obj.configMenuDict[newAid] = $li;
	...
	applyChange() {
		this.model = _additionalModel.model[KEY_ANCILLARY_GROUP];
		this.clearDirtyFlag();
	}
...
class MotorGroupEditor extends AbstractMainUi {
	constructor(device, gid, groupModel, seOnly) {
		super(device);
		this.gid = gid;
	...
	createUi() {
		...
		var motorList;
		if (obj.seOnly) {
			motorList = _motorDBModel.getSEMotorList();
		} else {
			motorList = _motorDBModel.getAllMotorList();
		}
		$.each(motorList, function(idx, motorName) {
...
//		});
		...
		obj.removeModal = new RemoveModal($del.find('span'), obj.device, obj.gid);
		obj.removeModal.init();
...
//			obj.$groupDesc.val('');
	...
	run() {
		...
		this.groups.addNewGroup(newGid, newDesc);
	}
	
}

class NewAncillaryModal extends AbstractModal {

	constructor($bt)
	{
		super($bt, 'id_modal_newAncillaryDialog');
		this.$ok = this.dialog.find('#id_button_newAncillary');
		this.$deviceId = this.dialog.find('#id_input_newAncillaryId');
	}
...
//			obj.$groupDesc.val('');
		...
		obj.$deviceId.keypress(function(event) {
			if ( event.key === "Enter" ) {
				if (obj.$deviceId.val().trim().length > 0) {
					obj.run();
				}
			}
		});
	...
	run() {
		this.close();
		const newAid = this.$deviceId.val();
		_ancillaryGroup.addNewDevice(newAid);
	}
...
class RemoveModal extends AbstractModal {

	constructor($bt, groups, gid)
	{
		super($bt, 'id_modal_deleteDialog');
		this.groups = groups;
		this.gid = gid;
		this.$confirm = this.dialog.find('#id_modal_deleteDialog_confirm');
	}
	...
	run() {
		this.close();
		this.groups.removeGroup(this.gid);
	}
}
...
var addPageTitle = function(){
	var titleString = "SICS Administration Configuration - " + _inst.charAt(0).toUpperCase() + _inst.slice(1);
	$(document).attr("title", titleString);
	var subTitle = _inst.charAt(0).toUpperCase() + _inst.slice(1) + " SICS Admin";
...
$(document).ready(function() {
	if (!_inst) {
		window.location = "sicsAdminMenu.html";
	}
	
	$('#id_a_signout').click(function() {
		signout("sicsAdmin.html?inst=" + _inst);
	});
