...
import de.kupzog.ktable.models.KTableSortComparator;
...
public class FixedCellRenderer extends DefaultCellRenderer {
    ...
    public FixedCellRenderer(int style) {
        super(style);
        m_Style |= STYLE_PUSH;
        borderColorHorizontal = COLOR_LINE_DARKGRAY;
        borderColorVertical =   COLOR_LINE_DARKGRAY;
        textFormatter = DefaultTextFormatter.getInst();
    }
    public FixedCellRenderer(int style, ITextFormatter textFormatter) {
    	this(style);
    ...
    public void drawCell(GC gc, Rectangle rect, int col, int row, Object content, boolean focus, boolean fixed, boolean clicked, KTableModel model) {
        ...
        Color bottomBorderColor = borderColorHorizontal;
        Color rightBorderColor =  borderColorVertical;
        ...
        if ((m_Style & STYLE_FLAT)!=0) {
            rect = drawDefaultSolidCellLine(gc, rect, rightBorderColor, bottomBorderColor);
    ...
    protected void drawCellContent(GC gc, Rectangle rect, int col, int row, Object content, KTableModel model, Color bgColor, Color fgColor) {
        ...
        if ((m_Style & INDICATION_ACTION) != 0 && model.getCellAction(col, row) != null) drawActionsSign(gc, rect);
    }
