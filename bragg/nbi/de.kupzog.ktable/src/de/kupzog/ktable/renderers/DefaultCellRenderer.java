...
import de.kupzog.ktable.Activator;
 ...
 */
public class DefaultCellRenderer implements KTableCellRenderer, Cloneable {
    //  default colors:
    ...
    public static Color COLOR_FGFOCUS = Display.getDefault().getSystemColor(SWT.COLOR_BLUE);
    public static Color COLOR_COMMENTSIGN = Display.getDefault().getSystemColor(SWT.COLOR_DARK_BLUE);
    public static Color COLOR_ACTIONSIGN = Display.getDefault().getSystemColor(SWT.COLOR_DARK_GREEN);
    ...
    /**
     * Рисует маленьки?треугольни??лево?верхне?углу
     */
    public static final int INDICATION_ACTION = 1<<9;
    ...
    protected Color borderColorVertical   = COLOR_LINE_LIGHTGRAY;
    protected Color borderColorHorizontal = COLOR_LINE_LIGHTGRAY;
    protected Color focusForeground =       COLOR_FGFOCUS;
    protected Color focusBackground =       COLOR_BGFOCUS;
    ...
    // [+] Accessors
    public Color getBorderColorVertical() {
        return borderColorVertical;
    }
    public void setBorderColorVertical(Color borderColorVertical) {
        this.borderColorVertical = borderColorVertical;
    }

    public Color getBorderColorHorizontal() {
        return borderColorHorizontal;
    }
    public void setBorderColorHorizontal(Color borderColorHorizontal) {
        this.borderColorHorizontal = borderColorHorizontal;
    }
    ...
    public Color getBackground() {
        if (m_bgColor != null) return m_bgColor;
    ...
    public int getOptimalWidth(GC gc, int col, int row, Object content, boolean fixed, KTableModel model) {
        applyFont(gc);
        int result = content == null ? 8 : SWTX.getCachedStringExtent(gc, content.toString()).x + 8;
        resetFont(gc);
        return result;
    }

    public int getWidthForText(GC gc, String text) {
        applyFont(gc);
        int result = SWTX.getCachedStringExtent(gc, text).x + 8;
        resetFont(gc);
        return result;
    }

    protected void drawFocus(GC gc, Rectangle rect) {
        int style = gc.getLineStyle();
        gc.setLineStyle(SWT.LINE_DOT);
        gc.setForeground(focusForeground);
    ...
    protected void drawCellContent(GC gc, Rectangle rect, String text, Image img, Color textColor, Color backColor) {
        ...
        SWTX.drawTextImage(
                gc, text, getAlignment(), 
                img, getAlignment(), rect.x + 3, rect.y + 2,
                rect.width - 6, rect.height - 3);
    }
    ...
    private int brightness(Color color) {
        ...
        return Math.max(Math.max(rgb.red, rgb.green), rgb.blue);
    }
    ...
    protected Rectangle drawDefaultSolidCellLine(GC gc, Rectangle rect, Color vBorderColor, Color hBorderColor) {
        boolean isVertical =
                (m_Style & SWT.HORIZONTAL) != SWT.HORIZONTAL &&
                ((m_Style & SWT.VERTICAL) == SWT.VERTICAL || brightness(borderColorVertical) < brightness(borderColorHorizontal));

        if (isVertical) {
            drawHorizontalBorder(gc, rect, hBorderColor);
            drawVerticalBorder(gc, rect, vBorderColor);
        } else {
            drawVerticalBorder(gc, rect, vBorderColor);
            drawHorizontalBorder(gc, rect, hBorderColor);
        }
    ...
    protected final void drawVerticalBorder(GC gc, Rectangle rect, Color color) {
    ...
    protected final void drawHorizontalBorder(GC gc, Rectangle rect, Color color) {
        gc.setForeground(color);
        gc.drawLine(
                rect.x,
                rect.y + rect.height,
    ...
    /**
     * Рисует символ доступны?действий ?лево?верхне?углу 
     * @param gc The GC to use when painting.
     * @param rect The cell area where content should be added.
     */
    protected final void drawActionsSign(GC gc, Rectangle rect) {
		gc.setBackground(COLOR_ACTIONSIGN);
//		gc.drawImage(Activator.getImage(Activator.IMG_CellActionSmall), rect.x + 1, rect.y+1);
	}
    ...
    protected void applyFont(GC gc) {
        m_GCfont = gc.getFont();
        if (m_font == null) m_font = Display.getCurrent().getSystemFont();
        if ((m_Style & SWT.BOLD) != 0 || (m_Style & SWT.ITALIC) != 0) {
            FontData[] fd = m_font.getFontData();
            int style = SWT.NONE;
            if ((m_Style & SWT.BOLD) != 0) style |= SWT.BOLD;
            if ((m_Style & SWT.ITALIC) != 0) style |= SWT.ITALIC;
    ...
    public DefaultCellRenderer clone() {
        try {
    ...
    public static void setAlignment(int alignment, DefaultCellRenderer...renderers) {
        for (DefaultCellRenderer rend: renderers) {
            rend.setAlignment(alignment);
        }
    }

    public static void setBorderColorHorizontal(Color color, DefaultCellRenderer...renderers) {
        for (DefaultCellRenderer rend: renderers) {
            rend.setBorderColorHorizontal(color);
        }
    }


    public static void setForeground(Color color, DefaultCellRenderer...renderers) {
        for (DefaultCellRenderer rend: renderers) {
            rend.setForeground(color);
        }
    }
    public static void setBackground(Color color, DefaultCellRenderer...renderers) {
        for (DefaultCellRenderer rend: renderers) {
