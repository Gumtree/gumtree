...
public interface KTableModel {
	 ...
	 */
	IKTableTooltip getTooltip(int col, int row);
	...
	/**
	 * ячейка будет "активной" если метод возвратит 
	 * action. ƒл€ "неактивных" €чеек возвращает null. 
	 * @param col
	 * @param row
	 * @return ¬озвращает объект KTableCellAction, который будет отвечать за действи€,
	 * инициируемые кликом мыши по фиксированной €чейке (col, row)
	 */
	KTableCellAction getCellAction(int col, int row);
    ...
    */
	 ...
	 * The clickable or selectable fixed columns start after getFixedHeaderRowCount() rows.
