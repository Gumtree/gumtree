...
public abstract class KTableCellEditor {
		...
		if (m_Control == null) {
			...
			m_Control.addFocusListener(new FocusAdapter() {
				public void focusLost(FocusEvent arg0) {
                    if (m_Control != null && !m_Control.isDisposed()) close(changed);
				}
			});
	...
	public void setBounds(Rectangle rect) {
		if (m_Control != null) m_Control.setBounds(rect);
	}
	...
	/**
	 * При включённом режиме активации редактора при вводе, данный метод позволяет проинициализировать
	 * его значением введённого с клавиатуры символа. Базовая реализация устанавливает флаг изменённости
	 * и вызывает метод setContent с ввёденным символом, преобразованным в строку в качестве параметра.
	 * @param ch Символ, введённый с клавиатуры при активации редактора.
	 */
	public void setPressedChar(char ch) {
	    changed = true;
        setContent(ch == 0 ? "" : (ch + ""));
	}
