...
public class SWTX {
	...
	public static class WeakCache {
	    private WeakReference<GC> last = new WeakReference<GC>(null);
	    private Map<String, Point> map = new HashMap<String, Point>();

        public Point get(GC gc, String text) {
            if (text == null) return new Point(0, 0);

            if (gc.equals(last.get())) {
	            Point p = map.get(text);
	            if (p != null) {
	                return new Point(p.x, p.y);
	            }
	        } else {
	            map.clear();
                last = new WeakReference<GC>(gc);
	        }
	...
	private static WeakCache CACHE_StringExtent = new WeakCache();
	/**
	 * @param gc Графический контекст
	 * @param text Текстовая строка
	 * @return габариты в пикселях указанной строки для данного графического контекста. При этом, расчитанные
	 * ранее габариты кэшируются для последнего значения графического контекста.
	 */
	public static synchronized Point getCachedStringExtent(GC gc, String text) {
	    return CACHE_StringExtent.get(gc, text);
	}
