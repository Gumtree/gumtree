...
//	Boolean geometryCorrection_skip = false;
	Boolean geometryCorrection_enable = true;
	...
	public Boolean process() throws Exception{
		if (!geometryCorrection_enable){
			...
			try{
				...
				ex.printStackTrace();
				thetaArray = geometryCorrection_scanData.findDataItem("thetaVector").getData();
			}
			// Make sure theta Array is 1D
			thetaArray = thetaArray.getArrayUtils().reduce().getArray();
//			if (((Plot) geometryCorrection_scanData).getAxis(drank-1).getName(
//					).contains("offset")) {
//				double stth = geometryCorrection_scanData.getDataItem("stth").getData().getMaximum();
//				thetaArray.add(stth);
//			}
			...
			double radius = 0.0;
			try {
				radius = geometryCorrection_scanData.findDataItem("radius").readScalarDouble();
			} catch (Exception e) {
		    ...
		    double det_height = geometryCorrection_scanData.getRootGroup().findDataItem("active_height").readScalarDouble();
			...
			IDataItem contribs = Factory.createDataItem(geometryCorrection_output, "contributors",out_contribs);
...
//			geometryCorrection_output.addStringAttribute(StaticDefinition.DATA_STRUCTURE_TYPE, 
	...
	public void setGeometryCorrection_enable(Boolean geometryCorrectionEnable) {
		geometryCorrection_enable = geometryCorrectionEnable;
	}
