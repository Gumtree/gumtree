...
public class Source extends ConcreteProcessor{
	...
	public Boolean process() throws Exception{
		...
		if(rawdata.getElementType()!=double.class){   // Need to convert to double
			...
			IArray doubledata = Factory.createArray(double.class, rawdata_shape);
			IArrayIterator raw_iter = rawdata.getIterator();
			IArrayIterator dbl_iter = doubledata.getIterator();
			while(raw_iter.hasNext()) {
				dbl_iter.next().setDoubleCurrent(raw_iter.getDoubleNext());
			}
		...
		if(need_to_replace) {
			...
			for (int new_axis_no=0;new_axis_no<data_axes.size();new_axis_no++)  {
				IDataItem oneaxis = data_axes.get(new_axis_no);
				int axis_no = Integer.parseInt(oneaxis.getAttribute("axis").getStringValue());
				for (int old_axis=0;old_axis<new_axis_no;old_axis++ ) 
					if (data_shape[old_axis]==1) axis_no--;
				IAttribute dim_attr = Factory.createAttribute("axis", String.format("%d",axis_no));
				oneaxis.addOneAttribute(dim_attr);
			}
		} else {
			replaced_group = source_asgroup;
		}
		error_data = rawdata.copy();
		dimensionType = getDataDimensionTye(replaced_group);
		...
		if (structureType == DataStructureType.plot){
			...
			try{
				PlotFactory.addDataVarianceToPlot(source_scanData,"dvariance",error_data);
			}catch (Exception e) {
				// TODO: handle exception
			}
