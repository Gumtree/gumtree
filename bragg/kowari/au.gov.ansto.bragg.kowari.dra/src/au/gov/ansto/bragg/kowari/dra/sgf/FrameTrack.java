...
public class FrameTrack extends ConcreteProcessor {
	...
	public Boolean process() throws Exception {
		...
		if (isInputChanged){
			...
			List<IDataItem> axes = NexusUtils.getNexusAxis(inputGroup);
			scanAxis = axes.get(0);
			boolean isTimeOfFlyAxis = false;
			for (int i = 0; i < axes.size() - 2; i++) {
				IDataItem item = axes.get(i);
				if (item.getSize() > 2) {
					scanAxis = item;
					if (i > 0) {
						isTimeOfFlyAxis = true;
					}
				}
			}
			...
			if (isTimeOfFlyAxis && frameNumber > 1) {
				frameNumber--;
			}
//			currentIndexVar = 0;
