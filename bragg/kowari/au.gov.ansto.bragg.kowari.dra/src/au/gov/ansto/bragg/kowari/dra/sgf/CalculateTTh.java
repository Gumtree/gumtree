...
//		if (units != null && units.equals("mm")){
		if (true){
			...
			if (maxStth == minStth){
				...
				String inputName = inputData.getShortName();
				if (inputName.startsWith("slice")){
					int endItem = 5;
					try {
						Integer.valueOf(inputName.substring(5, 6));
						endItem = 6;
						Integer.valueOf(inputName.substring(6, 7));
						endItem = 7;
						Integer.valueOf(inputName.substring(7, 8));
						endItem = 8;
					} catch (Exception e) {
					}
					if (endItem > 5) {
						int currentIndex = Integer.valueOf(inputName.substring(5, endItem));
						maxStth = maxStth / DEGREE_RAD_COEFFICIENT;
						double sinStth = Math.sin(maxStth);
						double cosStth = Math.cos(maxStth);
						twoThetaArray = Factory.createArray(Double.TYPE, new int[]{(int) binOffsetArray.getSize()});
