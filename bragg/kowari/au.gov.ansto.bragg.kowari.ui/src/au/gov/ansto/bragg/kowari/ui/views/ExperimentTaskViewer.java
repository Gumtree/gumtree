...
import au.gov.ansto.bragg.kowari.exp.task.AbstractKowariScanTask;
import au.gov.ansto.bragg.kowari.exp.task.AdvancedScanTask;
import au.gov.ansto.bragg.kowari.exp.task.HeaderInformationBlockTask;
import au.gov.ansto.bragg.kowari.exp.task.HmmscanTask;
import au.gov.ansto.bragg.kowari.exp.task.ScanNDTask;
import au.gov.ansto.bragg.kowari.exp.task.SicsBlockTask;
import au.gov.ansto.bragg.kowari.exp.task.AbstractKowariScanTask.ITaskPropertyChangeListener;
...
//		GridLayoutFactory.swtDefaults().numColumns(4).applyTo(designArea);
...
//		getToolkit().createLabel(timeGroup, "Seconds");
		...
		addSicsBlockButton = getToolkit().createButton(estimatedTimeComposite, "Add Sics Command", SWT.PUSH);
		try {
			addSicsBlockButton.setImage(SicsBatchUIUtils.getBatchEditorImage("ADD"));
		...
		GridDataFactory.fillDefaults().grab(true, false).applyTo(addSicsBlockButton);
...
//		});
	...
	private String getTimeString(float seconds){
		...
		if (seconds < 3600 * 2)
			return String.valueOf(((int) (seconds / 60)) + 1) + " minutes";
		if (seconds < 3600 * 5){
			...
			return String.valueOf((int) (seconds / 3600)) + " hours " + (
					minutes > 1 ? String.valueOf(minutes) + " minutes" : "");
		}
...
//				final ISicsCommandBlock commandBlock = ((CommandBlockTask) task).getDataModel();
//				if (commandBlock.getName() != null) {
				group.setText(((AbstractKowariScanTask) task).getTitle());
//				}
				Menu menu = group.getMenu();
				addManuItems(group, menu, (AbstractKowariScanTask) task);

				createTaskView(group, task);
				
				((AbstractKowariScanTask) task).addPropertyChangeListener(new ITaskPropertyChangeListener() {
...
//							).getEstimatedTime()));
			...
			if (task instanceof SicsBlockTask) {
...
//				final ISicsCommandBlock commandBlock = ((CommandBlockTask) task).getDataModel();
//				if (commandBlock.getName() != null) {
				group.setText(SicsBlockTask.TITLE);
//				}
				Menu menu = group.getMenu();
				addManuItems(group, menu, (SicsBlockTask) task);
				createTaskView(group, task);
...
//						saveTempWorkflow();
	...
	private void addManuItems(final MenuBasedGroup group, Menu menu, final CommandBlockTask avoidTask) {
		if (avoidTask instanceof AbstractKowariScanTask){
...
//		estimatedTime.setText(String.valueOf(0));
	...
	protected void addCommandBlock() {
		...
		if (lastTask instanceof AbstractKowariScanTask)
			getWorkflow().addTask(((AbstractKowariScanTask) lastTask).newThisTask());
		else
			getWorkflow().addTask(new ScanNDTask());
			// Refresh UI
		refreshUI();
	}
...
//			getWorkflow().insertTask(currentIndex - 1, task);
	...
	protected void addSicsBlock(){
		getWorkflow().addTask(new SicsBlockTask());
		refreshUI();
	}
