 ...
 *
 */
public class ChemistryPanel extends AbstractControlPanel {
	...
	public ChemistryPanel(Composite parent, int style, MainPart part) {
		...
		GridDataFactory.fillDefaults().grab(true, false).span(2, 1).minSize(320, 36).applyTo(titleLabel);
	    ...
	    final AbstractScanModel model = mainPart.getChemistryModel();
...
//		runButton.setSize(240, 64);
	...
	@Override
	public void show() {
		AbstractScanModel model = mainPart.getChemistryModel();
		if (model.getSize() == 0) {
			model.insertScan(0);
			table.redraw();
		}
