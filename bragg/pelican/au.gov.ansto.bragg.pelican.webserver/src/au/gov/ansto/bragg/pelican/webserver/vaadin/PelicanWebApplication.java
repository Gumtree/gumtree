package au.gov.ansto.bragg.pelican.webserver.vaadin;
...
public class PelicanWebApplication extends Application {

	private static final long serialVersionUID = -8015538517345159632L;

	private static final int REFRESH_INTERVAL = 5000;

	private static final Logger logger = LoggerFactory
			.getLogger(PelicanWebApplication.class);
	...
	public void init() {
		...
		context.window = new Window("Pelican Dashboard");
		...
		// Row 0
		Label label = new Label("Pelican Dashboard");
		...
		job = new Job("update") {
			...
			protected IStatus run(IProgressMonitor monitor) {
				try {
					...
					logger.error("Failed to update Pelican data", e);
				}
				if (PelicanWebApplication.this.isRunning()) {
