...
public class GSOMWorkflowViewer extends Composite {

	private ScriptExecutor Jython_Executor;
	private static final String GSOM_SCRIPT_NAME = "Internal/gsomScan.py";
	...
	public GSOMWorkflowViewer(Composite parent, int style) {
		super(parent, style);
...
//		GridDataFactory.fillDefaults().grab(true, false).hint(SWT.DEFAULT, 180).applyTo(consoleComposite);
		...
		level1Form.setWeights(new int[]{7, 4});
...
//		register.registerObject("Plot3", plot3Viewer);
		...
		controlViewer.loadScript(ScriptControlViewer.getFullScriptPath(GSOM_SCRIPT_NAME));
	}
