...
public class OldQuokkaScanView extends ViewPart {
	...
	private void createExperimentArea(Composite parent) {
		...
		try {
			IFileStore workflowConfig = EclipseUtils.find(
					...
					"workflows/MultiSampleExperiment.gwf");
			IWorkflow workflow = WorkflowFactory.createWorkflow(workflowConfig.openInputStream(EFS.NONE, new NullProgressMonitor()));
			commandLineViewer = new CommandLineViewer();
			workflow.getContext().put(ScriptEngineTask.CONTEXT_KEY_SCRIPT_VIEWER, commandLineViewer);
