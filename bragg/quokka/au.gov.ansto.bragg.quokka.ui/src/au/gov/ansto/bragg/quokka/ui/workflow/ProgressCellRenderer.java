 ...
 */
public class ProgressCellRenderer extends DefaultCellRenderer {
     ...
     * - INDICATION_GRADIENT
     */
    public ProgressCellRenderer(int style) {
        super(style);
    }
    ...
    protected void drawBar(GC gc, Rectangle rect, Object content, Color background, Color foreground) {
        ...
        else if (content instanceof ProgressData) {
        	 m_fraction = ((ProgressData)content).getProgress();
        	 text = ((ProgressData)content).getText();
        }
        else m_fraction=0;
        ...
        if (text != null) {
        	...
        	gc.drawText(text, rect.x, rect.y, true);
        }
    }
}
