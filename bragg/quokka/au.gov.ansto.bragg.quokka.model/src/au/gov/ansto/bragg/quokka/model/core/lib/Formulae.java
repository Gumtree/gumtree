...
*******************************************************************************/
package au.gov.ansto.bragg.quokka.model.core.lib;
...
//			d[i] = coefficient*((1/(var*Math.sqrt(2*pi)))*Math.exp(((-(x-mean)*(x-mean)))/2*var*var));	
	...
	private static Object generateGaussian(int[] size, double[] mean,
			double[] var, double[] increment, double coef) throws Exception {
	...
	private static double[][] generate2DGaussian(int[] dimension, double[] mean, double[] var, double[] increment, double coefficient){
		...
		double hInc = increment[0];
		double wInc = increment[1];
		double[] hVector = generateGaussian(width, mean[0], var[0], wInc);		
		double[] wVector = generateGaussian(height, mean[1], var[1], hInc);
		double[][] d = new double[height][width];
		for(int i=0; i<height; i++)
		{
			for(int j=0; j<width; j++)
	...
	private static double[][][] generate3DGaussian(int[] dimension, double[] mean, double[] var, double[] increment, double coefficient){
		...
		double[] hVector = generateGaussian(width, mean[0], var[0], increment[0]);		
		double[] wVector = generateGaussian(height, mean[1], var[1], increment[1]);
		double[] lVector = generateGaussian(length, mean[2], var[2], increment[2]);
		double[][][] d = new double[height][width][length];
		for(int i=0; i<height; i++)
		{
			for(int j=0; j<width; j++)
