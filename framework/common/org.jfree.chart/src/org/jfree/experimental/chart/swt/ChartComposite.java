...
import org.jfree.panning.Pannable;
...
public class ChartComposite extends Composite implements ChartChangeListener,
    ...
    /** Panning parameters */
    private org.eclipse.swt.graphics.Point panStartPoint;
    ...
    public void zoomInBoth(double x, double y, double zoomInFactor) {
        zoomInDomain(x, y, zoomInFactor);
        zoomInRange(x, y, zoomInFactor);
    }
    ...
    public void zoomInDomain(double x, double y, double zoomInFactor) {
        Plot p = this.chart.getPlot();
        if (p instanceof Zoomable)
        {
            Zoomable plot = (Zoomable) p;
            plot.zoomDomainAxes(zoomInFactor, this.info.getPlotInfo(),
                    translateScreenToJava2D(new Point((int) x, (int) y)));
        }
    }
    /**
    ...
    public void zoomInRange(double x, double y, double zoomInFactor) {
        Plot p = this.chart.getPlot();
        if (p instanceof Zoomable) {
            Zoomable z = (Zoomable) p;
            z.zoomRangeAxes(zoomInFactor, this.info.getPlotInfo(),
                    translateScreenToJava2D(new Point((int) x, (int) y)));
        }
    }
    /**
    ...
    public void zoomOutBoth(double x, double y, double zoomOutFactor) {
        zoomOutDomain(x, y, zoomOutFactor);
        zoomOutRange(x, y, zoomOutFactor);
    }
    ...
    public void zoomOutDomain(double x, double y, double zoomOutFactor) {
        Plot p = this.chart.getPlot();
        if (p instanceof Zoomable) {
            Zoomable z = (Zoomable) p;
            z.zoomDomainAxes(zoomOutFactor, this.info.getPlotInfo(),
                    translateScreenToJava2D(new Point((int) x, (int) y)));
        }
    }
    ...
    public void zoomOutRange(double x, double y, double zoomOutFactor) {
        Plot p = this.chart.getPlot();
        if (p instanceof Zoomable) {
            Zoomable z = (Zoomable) p;
            z.zoomRangeAxes(zoomOutFactor, this.info.getPlotInfo(),
                    translateScreenToJava2D(new Point((int) x, (int) y)));
        }
    }
...
//          this.canvas.addMouseWheelListener((MouseWheelListener) listener);
    ...
    public void mouseDoubleClick(MouseEvent event) {
    	...
    	if ((event.stateMask & SWT.CTRL) != 0){
    		zoomIn(translateScreenToJava2D(new Point((int) event.x, (int) event.y)), 0.7);
    	}
    	if ((event.stateMask & SWT.SHIFT) != 0){
			zoomOut(translateScreenToJava2D(new Point((int) event.x, (int) event.y)), 0.7);
    	}
    }
    ...
    public void mouseDown(MouseEvent event) {
    	...
    	if ((event.stateMask & SWT.CTRL) != 0){
            ...
            {
                org.eclipse.swt.graphics.Point point = getPointInRectangle(event.x, event.y, screenDataArea);
                panStartPoint = point;
                lastPanX = point.x;
                lastPanY = point.y;
            }
...
//                }
            ...
            {
           		pan(panStartPoint, lastPanX, lastPanY, event.x, event.y);
            }
            lastPanX = event.x;
            lastPanY = event.y;
    	}else{
    ...
    public void pan(org.eclipse.swt.graphics.Point start, int oldX, int oldY, int newX, int newY) {
    	...
    	if (p instanceof Pannable){
            Pannable panningPlot = (Pannable) p;
            Point2D startPoint = translateScreenToJava2D(new Point((int) start.x, (int) start.y));
...
//    private org.eclipse.swt.graphics.Point getPointInRectangle(int x, int y, Rectangle2D area)
//    {
...
//    }
	...
	public void mouseScrolled(MouseEvent event) {
		...
		if (increment < 0) 
			zoomIn(translateScreenToJava2D(new Point((int) event.x, (int) event.y)), zoomFactor);
		if (increment > 0) 
			zoomOut(translateScreenToJava2D(new Point((int) event.x, (int) event.y)), zoomFactor);
	}

    private void zoomIn(Point2D point, double zoomInFactor)
    {
        // ----------------
        ...
        for (int di = 0; di < getChart().getXYPlot().getDomainAxisCount(); di++)
        {
            ...
            domainAxis.zoomRange(domainRelPos * (1 - zoomInFactor), domainRelPos
                + zoomInFactor * (1 - domainRelPos));
        }
        ...
        for (int ri = 0; ri < getChart().getXYPlot().getRangeAxisCount(); ri++)
        {
            ...
            rangeAxis.zoomRange(rangeRelPos * (1 - zoomInFactor), rangeRelPos + zoomInFactor
                * (1 - rangeRelPos));
        }
    }
     ...
     */
    private void zoomOut(Point2D point, double zoomOutFactor)
    {
        // check if the mouse zoom was originated in a subplot
        int subPlot = getChartRenderingInfo().getPlotInfo().getSubplotIndex(point);
        ...
        for (int di = 0; di < getChart().getXYPlot().getDomainAxisCount(); di++)
        {
            ...
            domainAxis.zoomRange(-1 * domainRelPos * (1 - zoomOutFactor), 1.0
                + (1 - domainRelPos) * (1 - zoomOutFactor));
        }
        ...
        for (int ri = 0; ri < getChart().getXYPlot().getRangeAxisCount(); ri++)
        {
            ...
            rangeAxis.zoomRange(-1 * rangeRelPos * (1 - zoomOutFactor), 1.0 + (1 - rangeRelPos)
                * (1 - zoomOutFactor));
        }
