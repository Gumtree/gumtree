...
public class TabbedPropertyList
	...
	// [Tony]
	private int horizontalScaling = 1;
	
	public int getHorizontalScaling() {
		return horizontalScaling;
	}

	public void setHorizontalScaling(int horizontalScaling) {
	...
	public void setVerticalScaling(int verticalScaling) {
		this.verticalScaling = verticalScaling;
	}
	
	// [ANSTO][Tony][2011-06-17]
	private boolean showRightBorder = true;
	
	public boolean isShowRightBorder() {
		return showRightBorder;
	}

	public void setShowRightBorder(boolean showRightBorder) {
		this.showRightBorder = showRightBorder;
	}
...
//			if (selected && tab.getImage() != null
			...
			if (selected) {
				...
				// [Tony]
				e.gc.setFont(getBoldFont());
//				e.gc.setFont(JFaceResources.getFontRegistry().getBold(
//						JFaceResources.DEFAULT_FONT));
			}
			String text = tab.getText();
			// [Tony] limited wrap support (text is currently at the factor of 3)
//			if (getTextDimension(tab.getText()).x / 3 > bounds.width) {
//				text = text.substring(0, text.length() / 2) + "\n" + text.substring(text.length() / 2);
//				textMiddle = 3;
//			}
			
			e.gc.drawText(text, textIndent, textMiddle, true);
	...
	private Point getTextDimension(String text) {
		...
		GC gc = new GC(shell);
		// [Tony]
		gc.setFont(getNormalFont());
//		gc.setFont(JFaceResources.getFontRegistry().getBold(
//				JFaceResources.DEFAULT_FONT));
		Point point = gc.textExtent(text);
		point.x++;
		point.x *= getHorizontalScaling();
		point.y *= getVerticalScaling();
//		if (point.x > getBounds().width) {
//			point.y *= 3;	
//		}
		gc.dispose();
		shell.dispose();
