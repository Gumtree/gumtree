 ...
 * @since 3.3
 */
public class ApacheGaussianWithBackgroundFitter extends AbstractCurveFitter {
    /** Parametric function to be fitted. */
    private static final GaussianWithBackgroundFunction.Parametric FUNCTION = new GaussianWithBackgroundFunction.Parametric() {
            /** {@inheritDoc} */
            @Override
     ...
     */
    private ApacheGaussianWithBackgroundFitter(double[] initialGuess,
                                int maxIter) {
     ...
     */
    public static ApacheGaussianWithBackgroundFitter create() {
        return new ApacheGaussianWithBackgroundFitter(null, Integer.MAX_VALUE);
    }
     ...
     */
    public ApacheGaussianWithBackgroundFitter withStartPoint(double[] newStart) {
        return new ApacheGaussianWithBackgroundFitter(newStart.clone(),
                                       maxIter);
    }
     ...
     * @return a new instance.
     */
    public ApacheGaussianWithBackgroundFitter withMaxIterations(int newMaxIter) {
        return new ApacheGaussianWithBackgroundFitter(initialGuess,
                                       newMaxIter);
    }

    @Override
    public double[] fit(Collection<WeightedObservedPoint> points) {
    	Optimum optimum = getOptimizer().optimize(getProblem(points));
    	RealMatrix jacobian = optimum.getJacobian();
    	RealMatrix jacobianTrans = jacobian.transpose();
    	...
    	RealMatrix W = new DiagonalMatrix(target);
    	RealMatrix covariance = jacobianTrans.multiply(W).multiply(jacobian);
    	parameterErrors = new double[4];
    	for (int j = 0; j < parameterErrors.length; j++) {
    		parameterErrors[j] = 1 / FastMath.sqrt(covariance.getEntry(j, j));
		}
    ...
    protected LeastSquaresProblem getProblem(Collection<WeightedObservedPoint> observations) {
        ...
        double[] selfGuess = new ParameterGuesser(observations).guess();
        final double[] startPoint;
        if (initialGuess != null) {
        	startPoint = initialGuess.clone();
        	for (int j = 0; j < startPoint.length; j++) {
				if (Double.isNaN(startPoint[j])) {
					startPoint[j] = selfGuess[j];
				}
			}
        } else {
        	startPoint = selfGuess;
        }
...
//            new ParameterGuesser(observations).guess();
    ...
    public static class ParameterGuesser {
         ...
         */
        public double[] guess() {
            return new double[] { norm, mean, sigma, bg };
        }
        ...
        private double[] basicGuess(WeightedObservedPoint[] points) {
            ...
            final double j = points[minYIdx].getY();
//            final double k = points[minYIdx].getX();

            double fwhmApprox;
            try {
                final double halfY = (n - j) + ((m - (n - j)) / 2);
