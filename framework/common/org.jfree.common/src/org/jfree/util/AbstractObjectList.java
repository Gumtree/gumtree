...
public class AbstractObjectList implements Cloneable, Serializable {
    ...
    public void remove(int index) {
    	...
    	if (index >= 0 && index < size) {
    		final Object[] enlarged = new Object[size - 1];
    		if (index > 0) {
    			System.arraycopy(this.objects, 0, enlarged, 0, index);
    		}
    		if (index < size - 1) {
    			System.arraycopy(this.objects, index + 1, enlarged, index, size - index - 1);
    		}
            this.objects = enlarged;
            size = size - 1;
    	}
