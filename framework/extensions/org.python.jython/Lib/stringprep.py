...
# Jython note: this module is not shipped with the vendored Jython 2.7b1
# Lib, but encodings/idna.py (and therefore codecs.lookup('idna')) needs it.
# Two adjustments are made to the CPython original:
#   * unicode_literals, so the table entries below are unicode on Python 2
#     the same way the u'' prefixed literals of CPython 2.7 are;
#   * the unidata_version assert is dropped -- Jython's unicodedata here is
#     backed by UCD 4.1.0, not the 3.2.0 snapshot RFC 3454 nameprep is
#     defined against, so a handful of code points normalise differently.
