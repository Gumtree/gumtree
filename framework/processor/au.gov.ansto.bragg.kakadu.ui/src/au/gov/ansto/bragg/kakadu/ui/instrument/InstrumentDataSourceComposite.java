 ...
 * @author nxi
 */
public class InstrumentDataSourceComposite extends Composite {
...
//	private Label lblSelectedFile;
	...
	protected Action addDirectoryAction;
	protected Action combineSelectedFilesAction;
	protected Action removeFileAction;
	protected Action removeAllAction;
	protected Action deselectAllAction;
	protected Action selectAllAction;

	protected IWorkbenchAction dynamicHelpAction;




	public InstrumentDataSourceComposite(org.eclipse.swt.widgets.Composite parent, int style) {
		super(parent, style);
...
//		coolItemFileOperations.setSize(coolItemFileOperations.computeSize(toolBarSize.x, toolBarSize.y));
		...
		//init table tree
		fileTableTreeViewer = createTreeViewer();
...
//			TableTree tableTree = fileTableTreeViewer.getTableTree();
		...
		dataSourceTableLabelProvider = new SingleEntryLabelProvider();
	...
	protected CheckboxTableTreeViewer createTreeViewer() {
		return new CheckboxTableTreeViewer(this, SWT.FULL_SELECTION);
	}
	...
	public DataSourceFile addFile(String filePath, boolean willInformListener) {
		return addFile(filePath, willInformListener, -1);
	}
	/**
	 * Adds data file to the list of source files. 
	 * @param filePath absolute file path to the file.
	 */
	public DataSourceFile addFile(final String filePath, final boolean willInformListener, 
			final int location) {
...
//			}
		...
		final DataSourceFile sourcefile = dataSourceFile;
		DisplayManager.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				//create DataItem nodes
				DefaultMutableTreeNode fileNode = createNode(sourcefile);
				//add nodes to view
				if (location < 0)
					addFileNode(fileNode);
				else
...
//				fileTableTreeViewer.setSubtreeChecked(fileNode, true);
	...
	private DefaultMutableTreeNode createNode(DataSourceFile dataSourceFile) {
		...
		updateTableColumns(dataSourceFile.getArrributeNames());
...
//		}
	...
	private void insertFileNode(DefaultMutableTreeNode fileNode, int index) {
		fileTableTreeViewer.insert(getRootNode(), fileNode, index);
		getRootNode().insert(fileNode, index);
	}
...
//
	...
	public void setSortedColumn(int index) {
		...
		if (column != null) {
			dataSourceViewerSorter.setSortColumn(column);
		}
	}
}
