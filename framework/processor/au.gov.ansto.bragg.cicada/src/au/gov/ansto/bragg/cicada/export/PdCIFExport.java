...
public class PdCIFExport extends FormatedExport {
	...
	public void resultExport(URI fileURI, IGroup signal) throws IOException {
		...
		String block_name = new File(inputdata.getLocation()).getName();
		...
		try{
			...
			System.out.println("Now outputting Powder CIF file named " + newFile.getAbsolutePath());
			...
			String start_time = ((IDataItem) inputdata.getContainer("start_time")).getData().toString();
			String end_time = ((IDataItem) inputdata.findContainerByPath("$entry/end_time")).getData().toString();
			start_time = start_time.replace(" ", "T");
			end_time = end_time.replace(" ", "T");
			outputfile.format("_pd_block_id \t%s|%s|%s%n",block_name,current_time,sanitise_string(userName));
			String prog_details = extract_metadata(inputdata,"program_name") + " " + extract_metadata(inputdata,"$entry/sics_release");
			...
			outputfile.format("_pd_spec_special_details \t%s%n",extract_metadata(inputdata, "$entry/sample/name"));
			outputfile.format("_[local]_data_collection_description \t%s%n",extract_metadata(inputdata, "$entry/sample/description"));
			...
			outputfile.format("_pd_meas_info_author_email \t%s%n", extract_metadata(inputdata, "$entry/user/email"));
			outputfile.format("_pd_meas_info_author_phone \t%s%n", extract_metadata(inputdata, "$entry/user/phone"));
			outputfile.format("_pd_instr_2theta_monochr_pre \t%s%n",extract_metadata(inputdata, "$entry/instrument/crystal/takeoff_angle"));
			outputfile.format("_pd_instr_dist_mono/spec \t%s%n", extract_metadata(inputdata, "$entry/sample/mono_sample_mm","%.1f",1.0));
			outputfile.format("_pd_instr_dist_spec/detc \t%s%n",extract_metadata(inputdata,"radius","%.1f",1.0));
			...
			outputfile.format("_diffrn_source_power \t%s%n", extract_metadata(inputdata,"$entry/instrument/source/power","%.2f",1000));
			outputfile.format("_diffrn_radiation_probe   \tneutron%n");
			// Now get the instrument-specific information
			TreeMap<String,String> spec_metadata = new TreeMap<String, String>(inputdata.harvestMetadata("CIF"));
	...
	private String extract_metadata(Plot input_data, String first_location) {
		String first_string = "";
	...
	private String extract_metadata(Plot input_data, String first_location, String format_string, double scalefactor) {
		String first_string = "";
		try {
			IArray rawdata = null;
			Object item = input_data.getContainer(first_location);
