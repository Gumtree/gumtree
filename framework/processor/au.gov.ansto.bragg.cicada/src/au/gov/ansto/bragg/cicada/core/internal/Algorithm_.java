...
public class Algorithm_ extends Common_ implements Algorithm {
	...
	public Algorithm_(Algorithm algorithm) throws FailedToCloneException {
		...
		try {
			...
			throw new FailedToCloneException(
					"can not parse the recipe file " + algorithm.getFilename() + ": " + e.getMessage(), e);
		}
		...
		for (Iterator<Tuner> iter = this.tunerList.iterator(); iter.hasNext();) {
			if (sourceTunerIter.hasNext()) {
				...
				try {
					...
					throw new FailedToCloneException(
							"can not initiate tuner " + sourceTuner.getCoreName() + "\n" + e.getMessage(), e);
				}
			} else
				break;
		}
			...
			for (Iterator<Agent> iter = agentList.iterator(); iter.hasNext();) {
				...
				try {
					...
					throw new LoadAlgorithmFileFailedException(
							"can not set agent " + agent.getName() + ": " + e.getMessage(), e);
				}
			...
			for (Iterator<Tuner> iter = tunerList.iterator(); iter.hasNext();) {
				...
				if (ownerIDList != null) {
					for (Iterator<Integer> subIter = ownerIDList.iterator(); subIter.hasNext();) {
						...
						if (nextInteger.intValue() == tuner.getOwnerID())
							ownerID = nextInteger;
					}
					if (ownerID == null) {
	...
	public void loadAlgorithm(String fileLocation) throws LoadAlgorithmFileFailedException {
		try {
			...
			throw new LoadAlgorithmFileFailedException("can not find the algorithm configuration file " + fileLocation,
					e1);
		}
		...
		try {
			...
			throw new LoadAlgorithmFileFailedException(
					"can not read or parse the algorithm configuration file " + fileLocation, e);
		}
		try {
			frameworkConfiguration = ConfigurationFactory.createConfiguration(file);
		} catch (Exception e) {
			throw new LoadAlgorithmFileFailedException(
					"failed to create configuration from file " + fileLocation + "\n" + e.getMessage(), e);
		}
		try {
			framework = ProcessorFactory.loadAlgorithm(frameworkConfiguration);
		} catch (Exception e) {
			throw new LoadAlgorithmFileFailedException(
					"failed to create the processor framework for algorithm " + getName() + "\n" + e.getMessage(), e);
		}
		try {
			currentSourcePort = framework.getInList().get(0);
		} catch (Exception e) {
		...
		try {
			...
			throw new LoadAlgorithmFileFailedException(e.getMessage() + ": illegal framework name", e);
		}
		try {
			createTunerList();
		} catch (Exception e) {
	...
	public Document loadAlgorithmTitle(File algorithmFile) throws LoadAlgorithmFileFailedException {
		...
		try {
			...
			throw new LoadAlgorithmFileFailedException("failed to load or parse the algorithm from configuration file "
					+ algorithmFile.getName() + ": " + e.getMessage(), e);
		}
		Element rootElement = file.getRootElement();
		classID = rootElement.getAttributeValue("id");
		try {
			setName(rootElement.getAttributeValue("name"));
		} catch (IllegalNameSetException e) {
			throw new LoadAlgorithmFileFailedException(e.getMessage() + ": illegal algorithm name", e);
		}
		version = rootElement.getAttributeValue("version");
		helpURL = rootElement.getAttributeValue("help_url");
		shortDescription = rootElement.getAttributeValue("short_description");
		icon = rootElement.getAttributeValue("icon");
		try {
			algorithmType = AlgorithmType.valueOf(rootElement.getAttributeValue("type"));
		} catch (Exception e) {
		}
		String hiddenValue = rootElement.getAttributeValue("hidden");
		if (hiddenValue != null)
	...
	private boolean checkSourcePortExistence(Element rootElement) {
		...
		for (Element ins : rootElement.getChildren("ins")) {
			if (!ins.getChildren().isEmpty())
				return true;
		}
		return false;
	}
			...
			for (Iterator<Tuner> iter = tunerList.iterator(); iter.hasNext();) {
				...
				if (tuner.getName().contains("filename"))
					try {
						...
						//						e.printStackTrace();
						throw new SetTunerException(e.getMessage() + ": failed to set tuner", e);
					}
			}
		}
	}
		...
		if (currentSignal.getDatabag() != null && currentSourcePortList.size() > 0) {
			try {
				...
				throw new TransferFailedException(
						"failed to run the algorithm " + getName() + ": one of the step failed.\n" + e.getMessage(), e);
			} catch (ProcessorChainException e) {
				preHandleException();
				...
				throw new TransferFailedException(
						"failed to run the algorithm " + getName() + ": can not set the tuner.\n" + e.getMessage(), e);
			}
		} else {
			try {
	...
	public void subscribeExceptionHandler(ThreadExceptionHandler handler) {
		...
		if (!exceptionHandlerList.contains(handler))
			exceptionHandlerList.add(handler);
	}
	...
	public void unSubscribeExceptionHandler(ThreadExceptionHandler handler) {
		if (exceptionHandlerList != null)
			exceptionHandlerList.remove(handler);
	}
	...
	public void exportConfiguration(URI fileURI, String configurationName)
		...
		AlgorithmConfiguration configuration = new AlgorithmConfiguration(this, fileURI, configurationName);
	...
	public void exportPartialConfiguration(URI fileURI, String configurationName, String... tunerNames)
		...
		AlgorithmConfiguration configuration = new AlgorithmConfiguration(this, fileURI, configurationName, tunerNames);
		this.configuration = configuration;
		Exporter exporter = null;
		try {
			exporter = new Exporter_(new Formater_(Format.xml));
	...
	public void loadConfiguration(URI fileURI) throws LoadAlgorithmFileFailedException, ConfigurationException {
		...
		try {
			...
			throw new LoadAlgorithmFileFailedException("can not read from the target, check the file format", e1);
		}
		...
		try {
			...
			throw new LoadAlgorithmFileFailedException(e.getMessage() + ": failed to load cicada configuration file",
					e);
		}
			...
			if (tuner != null) {
				try {
					...
					throw new ConfigurationException(
							"can not set the signal to tuner " + tuner.getName() + "\n" + e.getMessage(), e);
				}
			}
		}
	...
	public AlgorithmConfiguration getConfiguration() throws ConfigurationException {
		if (configuration == null) {
			try {
				...
				configuration = new AlgorithmConfiguration(this, dataset, "algorithmConfiguration");
			} catch (IOException e) {
				throw new ConfigurationException("can not create configuration from the algorithm: " + e.getMessage(),
						e);
			}
		}
