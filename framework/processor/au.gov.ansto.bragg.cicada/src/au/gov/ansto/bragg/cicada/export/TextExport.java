...
public class TextExport extends FormatedExport{
...
//		bufferExport(bufferWriter, signal.getResultData());
	...
	private void bufferExport(BufferedWriter bufferWriter, Object signal, String title) throws IOException{
		...
		if (className.contains("Group")) bufferExportGroup(bufferWriter, (IGroup) signal, title);
		else if (className.contains("Variable")) bufferExportVariable(bufferWriter, (IDataItem) signal);
		else if (className.contains("Array")) bufferExportArray(bufferWriter, (IArray) signal);
