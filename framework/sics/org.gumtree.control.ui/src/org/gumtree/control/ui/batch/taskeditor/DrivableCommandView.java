...
public class DrivableCommandView extends AbstractSicsCommandView<DrivableCommand> {
		...
		Realm.runWithDefault(DisplayRealm.getRealm(Display.getDefault()), new Runnable() {
			public void run() {
				bindingContext.bindValue(
						...
						BeanProperties.value(DrivableCommand.class, "method").observe(getCommand()),
		...
		Realm.runWithDefault(DisplayRealm.getRealm(Display.getDefault()), new Runnable() {
			public void run() {
				bindingContext.bindValue(
						...
						BeanProperties.value(DrivableParameter.class, "deviceId").observe(parameter),
						new UpdateValueStrategy(),
						new UpdateValueStrategy()
				);
				bindingContext.bindValue(
						WidgetProperties.text(SWT.Modify).observe(targetText),
						BeanProperties.value(DrivableParameter.class, "target").observe(parameter),
						new UpdateValueStrategy(),
						new UpdateValueStrategy()
				);
	...
	private void validate(Text targetText, ControlDecoration controlDec, ComboViewer drivableComboViewer) {
		...
		ISicsController device = SicsManager.getSicsModel().findController((String) selectedDevice);
		if (device == null) {
			return;
		}
		IDynamicController upperlimController = (IDynamicController) device.getChild("/softupperlim");
		IDynamicController lowerlimController = (IDynamicController) device.getChild("/softlowerlim");
		if (upperlimController == null || lowerlimController == null) {
			return;
		}
		try {
			float upperlim = Float.parseFloat(String.valueOf(upperlimController.getValue()));
			float lowerlim = Float.parseFloat(String.valueOf(lowerlimController.getValue()));
