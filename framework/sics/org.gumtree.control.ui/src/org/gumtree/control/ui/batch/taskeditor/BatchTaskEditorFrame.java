...
import org.gumtree.control.ui.batch.BatchQueueViewer;
...
public class BatchTaskEditorFrame extends AbstractWorkflowViewerComponent {
	...
	public BatchTaskEditorFrame(Composite parent, int style) {
		super(parent, style);
		try {
	...
	protected void createUI() {
		...
		String bufferName = getWorkflow().getContext().get(
				VisualBatchScript.PROP_BUFFER_NAME, String.class);
		if (bufferName == null) {
		...
		BatchQueueViewer queueViewer = new BatchQueueViewer(
		...
		queueViewer.getViewer().addDoubleClickListener(new IDoubleClickListener() {			
			public void doubleClick(DoubleClickEvent event) {
				...
				IBatchScript buffer = (IBatchScript) selections.getFirstElement();
				if (buffer.getSource() instanceof IWorkflow) {
					// Remove old listener
							if (BatchTaskEditorFrame.this.buffer != null) {
								((AbstractModelObject) BatchTaskEditorFrame.this.buffer)
					...
					BatchTaskEditorFrame.this.buffer = buffer;
					((AbstractModelObject) BatchTaskEditorFrame.this.buffer)
...
//		sashForm.setMaximizedControl(composerGroup);
		IWorkflowViewerComponent controlViewer = new BatchEditorControlComposite(this, SWT.NONE);
