...
public class SicsSocketListener implements Runnable {
	...
	protected void handleConnectedState(String replyMessage) {
		if(replyMessage.equalsIgnoreCase(SicsCommunicationConstants.REPLY_LOGIN_OK))
...
//			getLogger().debug("Cannot interpret: " + replyMessage, e);
	...
	private Logger getLogger() {
		if(logger == null) {
			logger = LoggerFactory.getLogger(SicsSocketListener.class.getName() + ":" + getChannel().getChannelId());
		}
		return logger;
	}
