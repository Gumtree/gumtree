...
public class ControlView extends ViewPart {
	...
	public void dispose() {
		...
		if (filenameNotifyer != null) {
			filenameNotifyer.removeComponentListener(filenameListener);
	...
	private void createActions() {
		...
		bars.getToolBarManager().add(createLoadTclAction());
	}

	private Action createLoadTclAction() {
		Action action = new Action("Load Tcl") {
			public void run() {
				FileDialog dialog = new FileDialog(getSite().getShell(),
						SWT.OPEN);
				dialog.setFilterExtensions(new String[] { "*.tcl", "*.txt",
						"*.*" });
				String filename = dialog.open();
				if (filename != null) {
					setCommandTextFromFile(filename);
				}
			}
		};
		action.setImageDescriptor(Activator.imageDescriptorFromPlugin(
				Activator.PLUGIN_ID, "icons/tcl_obj.gif"));
		return action;
	}
