...
public class SicsBatchViewer extends AbstractPartControlProvider {
	...
	private void createBatchControlGroup(Composite parent) {
		...
		controlButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				BatchStatus status = SicsCore.getSicsManager().control().batch().getStatus();
				if (status.equals(BatchStatus.IDLE) || status.equals(BatchStatus.READY)) {
					start();
			...
			public void run() throws Exception {
				...
				if (status.equals(BatchStatus.IDLE) || status.equals(BatchStatus.READY)) {
					if (commands != null && commands.size() > 0) {
						try {
							SicsCore.getSicsManager().control().batch().run(commands.toArray(new String[commands.size()]), scriptName);
