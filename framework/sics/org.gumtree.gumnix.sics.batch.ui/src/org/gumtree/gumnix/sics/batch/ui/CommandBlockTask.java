...
import org.gumtree.gumnix.sics.batch.ui.internal.DndTransferData;
					...
					if (event.data instanceof StructuredSelection){
						if (((StructuredSelection) event.data).getFirstElement() 
							...
							if (parent == getTask()){
								...
								if (child instanceof ISicsCommandElement){
									ISicsCommandElement command = (ISicsCommandElement) child;
...
//									fireRefresh();
		...
		private void addCommandUI(Composite parent, final ISicsCommandElement command, 
				ISicsCommandView<? extends ISicsCommandElement> commandView) {
